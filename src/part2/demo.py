...
from src.part2.circ_hough import circ_hough
from src.part2.log_edge import log_edge
from src.part2.sobel_edge import sobel_edge
...
def demo_path(p: str) -> str:
    ...
    if os.path.isabs(p):
        return p
    pkg = (__package__.split(".")[-1]) if __package__ else "part2"
    # ensure we look under repo_root/src/<pkg>/...
...
def plot_log_sigmas(img: np.ndarray, sigmas: list):
    ...
    """
    assert len(sigmas) == 4, "Sigma list must contain exactly 4 elements for 2x2 grid."
...
def compare_sobel_and_log(img: np.ndarray, sobel_thres: float, log_sigma: float):
    """
...
def display_detected_circles(
    ...
    fig.suptitle("Detected Circles with Different Vote Thresholds", fontsize=16)
    fig.subplots_adjust(top=0.85)  # Adjust title position
