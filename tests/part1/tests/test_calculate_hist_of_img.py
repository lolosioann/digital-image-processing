...
def test_basic_histogram_counts():
    ...
    """
    img = np.array([[0.0, 0.5], [0.5, 1.0]])  # Image with pixel values 0.0, 0.5, 1.0
    expected = {0.0: 1, 0.5: 2, 1.0: 1}  # Expected histogram counts
    result = calculate_hist_of_img(
        img, return_normalized=False
    )  # Get histogram (counts)
    assert result == expected  # Assert that the result matches the expected counts
...
def test_all_same_pixel():
    ...
    expected_probs = {0.25: 1.0}  # Expected normalized probability for pixel value 0.25
    ...
    assert result_counts == expected_counts  # Assert that the counts match the expected
    assert np.isclose(
