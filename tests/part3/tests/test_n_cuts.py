...
def test_random_affinity_matrix():
    ...
    assert labels.shape[0] == 10  # We should have 10 nodes (rows) in the output
...
def test_disconnected_graph():
    # Create a disconnected graph with
    # affinity = 0 except diagonal (self-loops)
    ...
    assert len(np.unique(labels)) == 6  # Expecting 6 clusters (each node isolated)
