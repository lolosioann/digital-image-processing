...
def image_to_graph(img_array: np.ndarray) -> np.ndarray:
    """
    Converts an image to a fully-connected undirected graph represented by an affinity matrix.

    Args:
        img_array (np.ndarray): A float-type array of shape (M, N, C) with values in [0, 1].

    Returns:
        np.ndarray: A float-type 2D array of shape (M*N, M*N), representing the symmetric affinity matrix.
    """
    M, N, C = img_array.shape
    num_pixels = M * N

    # Flatten the image to shape (M*N, C), where each row is a pixel's intensity vector
