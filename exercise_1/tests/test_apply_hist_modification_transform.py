...
def test_missing_mapping_raises_error():
    ...
    transform = {
        0.1: 0.2  # 0.3 is missing
    }
    with pytest.raises(ValueError, match="Level 0.3 does not exist in the modification transform"):
        apply_hist_modification_transform(img, transform)


def test_input_not_2d():
...
def test_all_pixels_same_value():
