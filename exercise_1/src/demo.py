...
def display_img_hist_grid(
    ...
    if save_hist:
        title = (
            f"matched_hist_{mode}"
            if action == "matching"
            else f"equalized_hist_{mode}"
        )
...
input_img_path = "input_img.jpg"  # Path to the input image
ref_img_path = "ref_img.jpg"

input_img = Image.open(input_img_path).convert(
    "L"
...
ref_img = (
    np.array(ref_img, dtype=np.float64) / 255.0
